 ...
 * @author group_0739
 *
 *         This class represents a user of the system who can view, book, and
 *         sort through Itineraries flights. It is responsible for creating a
 *         new client of the program, adding and editing their personal and
 *         billing information, keeping track of and updating the itineraries
 *         they have booked, and searching for and sorting itineraries.
 */
public class Client extends User {
	...
	/**
	 * Constructs a new Client with a first and last name, info centre they
	 * belong to, email address and physical address, credit card number and
	 ...
	 * @param infoCentre
	 *            The info centre the Client belongs to.
	 * @param email
	 ...
	 * @param expiryDate
	 *            The date the Client's credit card expires in format:
	 *            YYYY-MM-DD.
	 */
	public Client(String first, String last, Infocentre infoCentre, String email, String address, long creditCardNumber,
			String expiryDate) {
		...
		try {
			if (expiryDate.length() == 10) {
				...
				if (expiryDate.charAt(4) == '-' && expiryDate.charAt(7) == '-') {
					...
					/// parseInt throws an exception if the string is not
					/// numeric
					...
					if (expiryYear >= 0 && expiryMonth >= 1 && expiryMonth <= 12 && expiryDay >= 1 && expiryDay <= 31) {

						// empty if - all input is valid
					} else {
						throw new InvalidInputException("Year, " + "Month or Dayare invalid.");
					}
				} else {
					throw new InvalidInputException("Expiry Date must be in the" + " format: YYYY-MM-DD");
				}
			} else {
				throw new InvalidInputException("Expiry Date must be in the" + " format: YYYY-MM-DD");
			}
	 ...
	 * 
	 * @param email
	 *            The Client's new email address.
	 */
	 ...
	 * 
	 * @param creditCardNumber
	 *            The Client's new credit card number.
	 */
	 ...
	 * 
	 * @param filter
	 *            The sorting filter of the Client's itineraries.
	 */
	public void setFilter(String filter) {
		try {
			if (filter.equals("cost") || filter.equals("travel time")) {
				...
				throw new InvalidInputException("Filter must be either " + "cost or travel time.");
			}
		} catch (InvalidInputException e) {
	 ...
	 * 
	 * @param expiryDate
	 *            The new expiry date of the Client's credit card in the format:
	 *            YYYY-MM-DD.
	 */
			...
			if (expiryDate.length() == 10) {
				...
				if (expiryDate.charAt(4) == '-' && expiryDate.charAt(7) == '-') {
					...
					if (expiryYear >= 0 && expiryMonth >= 1 && expiryMonth <= 12 && expiryDay >= 1 && expiryDay <= 31) {
						...
						throw new InvalidInputException("Year, Month " + "or Day are invalid.");
					}
				} else {
					throw new InvalidInputException("Expiry Date must " + "be in the format: YYYY-MM-DD");
				}
	 ...
	 * 
	 * @return A String representing the Client's personal and billing
	 *         information.
	 */
	public String toString() {
		return this.getLastName() + "," + this.getFirstName() + "," + this.getEmail() + "," + this.getAddress() + ","
